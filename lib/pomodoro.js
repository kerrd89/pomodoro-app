...
class Pomodoro {
  ...
  ifTimeExpired(n) {
    if (this.timers[n].timeLeft <= 0) {
      ...
      this.timers[nextTimer].currentState = 'active';
  ...
  appendToPage() {
    this.getStorage();
    if (this.timers.length > 0) { this.appendNewTimers(3); }
    else {
      for (let i = 0; i < this.timers.length; i++) {
        if (this.timers[i].type === 'work') {
          let timeLeft = Math.floor(this.timers[i].timeLeft / 60000);
          this.appendWorkTimerToPageIfSomethingInStorage(i, timeLeft);
          console.log(this.timers[i].currentState); }
        else {
          let timeLeft = Math.floor(this.timers[i].timeLeft / 60000);
          this.appendBreakTimerToPageIfSomethingInStorage(i, timeLeft);
          console.log(this.timers[i].currentState);
        }
      }
    }
  ...
  appendWorkTimerToPageIfSomethingInStorage(i, timeLeft) {
    $('ul').append(`
  ...
  appendBreakTimerToPageIfSomethingInStorage(i, timeLeft) {
    $('ul').append(`
      <li class="${this.timerStatus(i)}">
  ...
  appendNewTimers(amountToCreate) {
    let amountLeft = amountToCreate;
    $('ul').append(`
    ...
    `);
    amountLeft--;
    if (amountLeft > 0) { this.appendNewTimers(amountLeft); }
  }
